...
\addlinespace
\textbf{v6.0-3site} (ours) & \textbf{0.954} & [pending] & 0.201 & [pending] & \textbf{75.25} & [pending] & \textbf{no} \\
\bottomrule
\end{tabular}
\end{table}

The v6.0-3site row (bottom) reports results from the 3-site training run that completed concurrent with this draft. Adding PadChest-GR as a third training site pushes IMG from 69.24pp to 75.25pp and improves validation accuracy from 0.925 to 0.954. This is the headline training configuration for the remainder of the paper.
...
\paragraph{Results.} Current silver labels (RadFact + VERT on 2{,}707 claims, with GREEN still running) produce the following agreement counts: 1{,}598 claims unanimously \textsc{SUPPORTED}, 259 unanimously \textsc{CONTRADICTED}, 283 unanimously \textsc{UNCERTAIN}. Conflict rate (RadFact and VERT disagree on SUP/CONTRA/UNCERTAIN): 13\%. The baseline landscape is reported in Table~\ref{tab:baselines}: among the four public detectors that completed on the real-RRG subset, MedGemma-4B-IT shows IMG = 14pp but ESG = 2pp (evidence-blind on the shuffled-evidence axis), BiomedCLIP zero-shot shows IMG = 8pp and ESG = 0pp (borderline image-use; ignores evidence entirely), and our trained v6.0-3site verifier reaches IMG $\approx 75$pp. Two API detectors (Claude 3.5 Sonnet, MAIRA-2 as verifier) returned 0.0 accuracy on this run, attributable to image preprocessing mismatch between verifier-mode prompts and the MAIRA-2 processor API (and, for Claude, to per-minute rate limits on Opus-4-7). Both are pending re-runs.
...
We evaluate whether the evidence-blindness mitigation transfers across sites by training three held-out-site configurations on the 3-site corpus: held-out OpenI (train on ChestX-Det10 + PadChest-GR), held-out ChestX-Det10 (train on OpenI + PadChest-GR), and held-out PadChest-GR (train on OpenI + ChestX-Det10). All three use the v5.3-contrastive loss composition and are trained for two epochs with the HO-filter weights re-computed on each training set (rather than borrowed from the full 3-site weights, to preserve row-index alignment).
...
\begin{tabular}{lccc}
\toprule
\textbf{Held-out site} & \textbf{IMG (held-out)} & \textbf{Acc (held-out)} & \textbf{Blind?} \\
\midrule
OpenI         & [pending] & [pending] & [pending] \\
ChestX-Det10  & [pending] & [pending] & [pending] \\
PadChest-GR   & [pending] & [pending] & [pending] \\
\midrule
...
RadFlag replication            & [pending] & [pending] & [pending] & [pending] \\
ReXTrust-inspired probe (MedVersa / MAIRA-2) & [pending] & [pending] & [pending] & [pending] \\
\textbf{ClaimGuard-CXR v6.0-3site} (ours) & \textbf{0.954} (val) & $\approx 75$ & [pending] & \textbf{no} \\
\bottomrule
\end{tabular}
\end{table}
...
The benchmark is compact: approximately 7{,}500 images from two public sources, yielding 25{,}176 labeled training claims. The per-category IMG analysis is stable at this scale but the per-pathology cells with small n are noisier than the aggregate numbers. A larger benchmark would tighten the per-category table without changing the headline claim.
