...
PadChest-GR ships 11{,}995 radiologist-labeled sentences (7{,}037 positive-finding with bounding boxes, 4{,}958 explicitly-absent from the same radiologist's reports). These constitute the only public claim-level radiologist-grounded set in chest radiology not derived from MIMIC-CXR. The intended validation is per-image pairing — for each PadChest-GR sentence, find the best-aligned silver-labeled claim on the same image and compute agreement — which requires silver labels generated on PadChest-GR images themselves. Because we did not complete the PadChest-GR claim-extraction step (§\ref{sec:limitations}) in time for this submission, our silver labels live on OpenI images only and the \texttt{image\_id} pairing step returned zero matches (details in supplement). An alternative text-only matching pass (PadChest-GR sentence text vs OpenI-image silver claim text across all images) is possible but measures semantic phrasing agreement rather than radiologist-vs-silver grounding agreement, and is not a substitute. The full paired validation is deferred to the camera-ready; the pipeline is scaffolded at \texttt{v5/eval/padchest\_gr\_validate.py}.
...
We audit the claim-only text shortcut directly by training a RoBERTa-large classifier on the claim text only (no image, no evidence), once on the raw training distribution and once on the HO-filter-downweighted distribution. On the 4{,}132-claim v6 test split, the pre-filter text-only model reaches $82.0\%$ (vs majority-class $87.6\%$, $-5.6$pp); the post-filter text-only model reaches $79.5\%$ ($-8.1$pp vs majority). The HO filter reduces text-only shortcut performance by $2.5$ percentage points without changing the training-data row count (the filter downweights rather than drops). Both pre- and post-HO text-only accuracies fall below the majority-class baseline, which means the v6 training distribution is already largely free of the claim-only text shortcut — there is no trivial way to guess \texttt{SUPPORTED/CONTRADICTED} from the claim sentence alone. The HO filter's contribution on the artifact-audit axis is therefore small in absolute terms; its primary contribution is visible in the loss-drop ablation (removing the HO filter alone collapses IMG from $62.9$pp to $5.8$pp on v5.2), not in the text-only-vs-majority gap reported here.
...
\paragraph{RadFlag replication} \citep{zhang2024radflag}. A black-box temperature-sampling detector is scaffolded at \texttt{v5/eval/radflag\_replica.py}. For each generated claim, the procedure samples $K=10$ temperature-varied generations of the underlying RRG model and entails the original claim against each sample via Claude Opus; inconsistency rate is the flag signal. Applying RadFlag to MAIRA-2 outputs requires roughly $20{,}000$ Claude Opus entailment calls at current rate limits; this is feasible but falls outside the pre-submission compute window.

\paragraph{ReXTrust-inspired white-box probe} \citep{hardy2024rextrust}. A linear probe on generator hidden states, reimplemented from the published description (no public code or weights), scaffolded at \texttt{v5/eval/hidden\_state\_probe.py}. We plan to apply it to both MedVersa \citep{zhou2024medversa} (the paper's backbone) and MAIRA-2 to argue the result is architecture-independent. The probe requires approximately $6$ H100-hours on each backbone.
