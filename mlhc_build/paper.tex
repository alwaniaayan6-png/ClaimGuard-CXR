...
\centering
\small
\caption{Evidence-blindness diagnostic on a 2{,}906-claim test split. IMG = image-masking gap, ESG = evidence-shuffling gap, IPG = image-perturbation gap on laterality-turning subset. A verifier is evidence-blind if IMG $< 5$pp or ESG $< 5$pp.}
\label{tab:headline}
...
\paragraph{Results.} [Results table pending final silver-ensemble output; current RadFact + VERT partial labels show 1{,}598 agreed \textsc{SUPPORTED}, 259 agreed \textsc{CONTRADICTED}, and 283 agreed \textsc{UNCERTAIN} on the gated subset; a further 368 CheXagent claims labeled in parallel.]
...
[Table of per-detector accuracy and IMG/ESG/IPG pending; expectation per our working hypothesis: at least five of seven detectors exhibit IMG $< 5$pp on real-RRG claims, establishing evidence-blindness as a field-level property rather than a single-model artifact.]
