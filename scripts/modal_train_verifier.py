"""Modal script for training the ClaimGuard-CXR claim verifier on cloud GPUs.

Usage (from local Mac):
    modal run --detach scripts/modal_train_verifier.py

Trains a RoBERTa-large cross-encoder verifier (+ CNN heatmap encoder that
currently sees zero heatmaps — text-only in practice) on the 8 hard-negative
v2 training data, with patient-stratified val split, best-on-val-loss
checkpointing, label smoothing, and full determinism enforcement.

Fixes vs v1:
  - C12: patient-stratified 10% val split + best-on-val-loss + early stop
  - M1: label smoothing 0.05 on CE loss
  - M11: cudnn.deterministic + use_deterministic_algorithms
  - Updated docstring (was mislabeled DeBERTa-v3)
"""
...
app = modal.App("claimguard-verifier")
...
def train_verifier(
    ...
    output_dir: str = "/data/checkpoints/verifier",
    ...
    class VerifierDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            label = item["label"]  # 0=Supported, 1=Contradicted, 2=Insufficient
    ...
    class VerifierModel(nn.Module):
        def __init__(self, model_name, heatmap_dim=768, num_classes=3, hidden_dim=256, dropout=0.1):
            super().__init__()
    ...
    # Loss (M1 FIX: label smoothing to reduce overconfidence + improve calibration)
